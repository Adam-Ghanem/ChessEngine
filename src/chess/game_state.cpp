...
void updateCastlingRights(std::string& rights, Color color, Square from, Square to, Piece captured) {
    ...
    if (captured == Piece::WhiteRook && to == Square::A1) erase('Q');
    if (captured == Piece::WhiteRook && to == Square::H1) erase('K');
    if (captured == Piece::BlackRook && to == Square::A8) erase('q');
    if (captured == Piece::BlackRook && to == Square::H8) erase('k');
}
