...
struct TTEntry { std::uint64_t key = 0; int depth = -1; int score = 0; Bound bound = Bound::Exact; Move best{}; };
...
int pawnStructure(const Position& p) noexcept {
    ...
    for (int file = 0; file < 8; ++file) {
        ...
        const Bitboard adjacent = (file > 0 ? fileMask(file - 1) : 0ULL) | (file < 7 ? fileMask(file + 1) : 0ULL);
    ...
    for (int sq = 0; sq < 64; ++sq) {
        ...
        const int file = sq & 7;
        const int rank = sq >> 3;
        const Bitboard frontFiles = fileMask(file) | (file > 0 ? fileMask(file - 1) : 0ULL) | (file < 7 ? fileMask(file + 1) : 0ULL);
        if (wp & bit) {
            const int frontRank = rank + 1;
            if (frontRank >= 8 || (bp & frontFiles & ~((1ULL << (8 * frontRank)) - 1ULL)) == 0)
                score += 18 + rank * 8;
        }
        if (bp & bit) {
            const int frontRank = rank - 1;
            if (frontRank < 0 || (wp & frontFiles & ((1ULL << (8 * (frontRank + 1))) - 1ULL)) == 0)
                score -= 18 + (7 - rank) * 8;
        }
    }
    return score;
...
struct Engine::Impl {
    ...
    int evaluateAbsolute(const Position& p) const noexcept {
        ...
        for (int pi = 1; pi <= 12; ++pi) {
            ...
            if (pi != 1 && pi != 6 && pi != 7 && pi != 12) nonPawnMaterial += std::popcount(bb) * std::abs(value);
            while (bb) {
                const int sq = static_cast<int>(std::countr_zero(bb));
    ...
    int moveIndex(const Move& m) const noexcept { return static_cast<int>(m.from()) * 64 + static_cast<int>(m.to()); }
    ...
    std::vector<Move> orderedMoves(const Position& p, std::vector<Move> moves, int ply, const Move& ttMove) const {
        std::stable_sort(moves.begin(), moves.end(), [&](const Move& a, const Move& b) { return moveScore(p, a, ply, ttMove) > moveScore(p, b, ply, ttMove); });
        return moves;
    }
    ...
    int negamax(GameState& state, int depth, int alpha, int beta, int ply, std::vector<Move>& pv) {
        ...
        if (legal.empty()) return isInCheck(state.position()) ? -MateScore + ply : 0;
        ...
        for (const Move& m : moves) {
            ...
            if (moveNumber == 0) {
                ...
                if (score > alpha && score < beta) score = -negamax(state, childDepth, -beta, -alpha, ply + 1, childPv);
            }
            state.unmakeMove();
            if (stopFlag.load(std::memory_order_relaxed)) return 0;
            if (score > bestScore) {
            ...
            if (alpha >= beta) {
                if (!m.isCapture() && !m.isPromotion()) {
                    if (ply < MaxPly && killers1[ply] != m) { killers2[ply] = killers1[ply]; killers1[ply] = m; }
                    const int side = state.position().sideToMove() == Color::White ? 0 : 1;
                    history[side][moveIndex(m)] = std::min(50'000, history[side][moveIndex(m)] + depth * depth);
                }
                break;
            }
            ++moveNumber;
    ...
    SearchResult run(const Position& root, SearchLimits limits) {
        stopFlag.store(false, std::memory_order_relaxed); nodes = 0; timed = false;
        if (limits.moveTimeMs > 0) {
            timed = true; deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(limits.moveTimeMs);
        } else {
            ...
            if (remaining > 0) {
                ...
                timed = true; deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(budget);
            }
        }
        ...
        for (int depth = 1; depth <= maxDepth; ++depth) {
            ...
            GameState state(root); std::vector<Move> pv;
            int alpha = -Infinity, beta = Infinity;
            if (depth >= 4 && std::abs(previousScore) < MateThreshold) { alpha = previousScore - 50; beta = previousScore + 50; }
            int score = negamax(state, depth, alpha, beta, 0, pv);
            if (!stopFlag.load(std::memory_order_relaxed) && (score <= alpha || score >= beta)) {
                state = GameState(root); pv.clear(); score = negamax(state, depth, -Infinity, Infinity, 0, pv);
            }
            if (stopFlag.load(std::memory_order_relaxed)) break;
            if (!pv.empty()) { result.bestMove = pv.front(); result.principalVariation = pv; result.score = score; result.depth = depth; previousScore = score; }
            result.nodes = nodes;
...
std::string Engine::pvToUci(const std::vector<Move>& pv) {
    ...
    for (std::size_t i = 0; i < pv.size(); ++i) { if (i) out << ' '; out << pv[i].toUci(); }
    return out.str();
}
