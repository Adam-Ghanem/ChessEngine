...
export default function Home() {
  ...
  function showNextReview() { selectMove(Math.min(moves.length - 1, (reviewIndex ?? activeIndex) + 1), true); }
  function shareReview() { toast("Share export will be available from saved analysis."); }
      ...
      <div className="analysis-product-shell">
        <header className="app-header product-header">
          ...
          <div className="header-actions">
            <button className="icon-button" aria-label="Open ChessIQ settings" onClick={() => toast("Settings panel is coming next.")}><Settings2 size={18} /></button>
        ...
        <section className="analysis-hero">
          <div className="analysis-hero-copy"><div className="analysis-hero-kicker"><Sparkles size={14} /> ChessIQ Review</div><h1>Understand every move.</h1><p>Navigate the game, inspect the engine line, and turn each critical position into something you can learn from.</p></div>
          <div className="analysis-hero-meta" aria-label="Current analysis status">
            ...
            <div className={isAnalyzing ? "engine-status is-live" : "engine-status"}><span>Engine</span><strong>{isAnalyzing ? "Calculating" : liveAnalysis ? `Live · d${liveAnalysis.depth}` : "Ready"}</strong></div>
          </div>
        </section>
        ...
        <div className="analysis-layout">
          <section className="analysis-main-column">
            <section className="analysis-board-card">
              <header className="analysis-card-header"><div><span className="analysis-label">Current position</span><h2>{title}</h2></div><div className="mode-switch" role="group" aria-label="Analysis detail mode"><button className={mode === "beginner" ? "is-active" : ""} onClick={() => setMode("beginner")}>Beginner</button><button className={mode === "advanced" ? "is-active" : ""} onClick={() => setMode("advanced")}>Advanced</button></div></header>
              <div className="analysis-board-stage"><div className="analysis-board-wrap"><EvaluationBar evaluation={{ ...currentMove.evaluation, score: liveScore }} /><ChessBoard fen={currentMove.fen} lastMove={{ from: currentMove.from, to: currentMove.to }} engineArrow={liveArrow} classification={currentMove.classification} showClassificationMarker={reviewIndex === activeIndex} /></div></div>
              <div className="analysis-transport" aria-label="Move navigation">
                <div className="analysis-move-summary"><span>Move {currentMove.fullMove}{currentMove.side === "black" ? "…" : "."}</span><strong>{currentMove.san}</strong><ClassificationBadge classification={currentMove.classification} /></div>
                <div className="transport-controls"><button className="transport-button" aria-label="Go to first move" disabled={!canGoBack} onClick={() => selectMove(0)}><ChevronsLeft size={19} /></button><button className="transport-button" aria-label="Previous move" disabled={!canGoBack} onClick={() => selectMove(activeIndex - 1)}><ChevronLeft size={20} /></button><button className="transport-button transport-play" aria-label="Review selected position" onClick={() => setReviewIndex(activeIndex)}><Play size={16} fill="currentColor" /></button><button className="transport-button" aria-label="Next move" disabled={!canGoForward} onClick={() => selectMove(activeIndex + 1)}><ChevronRight size={20} /></button><button className="transport-button" aria-label="Go to final move" disabled={!canGoForward} onClick={() => selectMove(moves.length - 1)}><ChevronsRight size={19} /></button></div>
              </div>
              {reviewIndex !== null && <div className="analysis-review-inline"><GameReviewCard key={reviewIndex} move={moves[reviewIndex]} onClose={() => setReviewIndex(null)} onNext={showNextReview} onShare={shareReview} /></div>}
            </section>
            <section className="analysis-moves-card"><div className="analysis-section-heading"><div><span className="analysis-label">Game timeline</span><h2>Moves</h2></div><span>{moves.length} ply</span></div><MoveList moves={moves} activeIndex={activeIndex} onSelect={(index) => selectMove(index, ["BRILLIANT", "GREAT", "BEST", "MISTAKE", "BLUNDER"].includes(moves[index].classification))} /></section>
          </section>

          <aside className="analysis-side-stack">
            <section className="analysis-command-card"><div className="analysis-section-heading"><div><span className="analysis-label">Engine review</span><h2>Analyze</h2></div><Gauge size={19} /></div><button className="primary-action analysis-primary-action" onClick={startAnalysis} disabled={isAnalyzing}><BarChart3 size={17} />{isAnalyzing ? "Calculating…" : liveAnalysis ? "Analyze again" : "Analyze position"}</button>{analysisError && <p className="analysis-inline-error" role="alert">{analysisError}</p>}<div className="analysis-quick-actions"><button aria-label="Toggle analysis detail" onClick={() => setMode((current) => current === "beginner" ? "advanced" : "beginner")}><SlidersHorizontal size={17} /> Detail</button><button aria-label="Import a PGN" onClick={() => toast("PGN import will be available from the Games workspace.")}><FileUp size={17} /> Import</button><button aria-label="Export analyzed game" onClick={() => toast("Export will be available after an analysis is saved.")}><Download size={17} /> Export</button></div></section>
            <AnalysisPanel move={currentMove} mode={mode} isAnalyzing={isAnalyzing} liveAnalysis={liveAnalysis} />
            <section className="analysis-insight-card" aria-labelledby="explanation-heading"><div className="analysis-section-heading compact"><div><span className="analysis-label">ChessIQ insight</span><h2 id="explanation-heading">Why this move matters</h2></div><Target size={18} /></div><div className="analysis-insight-title"><ClassificationBadge classification={currentMove.classification} /><strong>{currentMove.fullMove}{currentMove.side === "black" ? "…" : "."} {currentMove.san}</strong></div><p>{liveAnalysis ? `Live ChessEngine search at depth ${liveAnalysis.depth}. Principal variation: ${liveAnalysis.principalVariation || "not returned"}.` : currentMove.explanation}</p><div className="analysis-best-move"><span>Best move</span><strong>{liveBestMove}</strong></div>{activeMoment?.practiceIndex !== undefined && <button className="inline-try-again" onClick={() => startTryAgain(activeMoment.practiceIndex!)}><RotateCcw size={13} /> Try again from here</button>}</section>
            ...
            <section className="analysis-graph-card"><div className="analysis-section-heading compact"><div><span className="analysis-label">Evaluation</span><h2>Game trend</h2></div><span>{modeLabel}</span></div><EvaluationGraph moves={moves} activeIndex={activeIndex} onSelect={selectMove} /></section>
          </aside>
        </div>
