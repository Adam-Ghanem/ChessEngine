...
const premiumCss = readFileSync(new URL("./play-premium-polish.css", import.meta.url), "utf8");
...
describe("Play mobile action dock navigation clearance", () => {
  it("keeps Play actions above the fixed ChessIQ bottom navigation and safe area", () => {
    ...
    expect(premiumCss).toMatch(/\.play-product-shell \.play-mobile-actions\s*\{[^}]*bottom:calc\(84px \+ env\(safe-area-inset-bottom,0px\)\)/);
  });
});
